...
class MainWindow(QWidget):
    ...
    def _change_day_from_table(self, rowNum, day):
        ...
        try:
            ...
            lol=0
            self.cursor.execute("SELECT * FROM subject;")
            recordss = list(self.cursor.fetchall())
            for i in recordss:
    ...
    def _delete_subject_from_table(self, rowNum):
        ...
        for i in range(a.columnCount()):
            try:
                ...
                row.append(None)
        try:
            self.cursor.execute(f"SELECT COUNT(*) as count FROM timetable WHERE subject='{row[0].encode('cp866').decode('cp1251')}';")
            records = list(self.cursor.fetchall())
            ...
            self.cursor.execute(f"SELECT COUNT(*) as count FROM teacher WHERE subject='{row[0].encode('cp866').decode('cp1251')}';")
            records = list(self.cursor.fetchall())
            if records[0][0] > 0:
